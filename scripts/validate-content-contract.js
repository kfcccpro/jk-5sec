...
if (!contract || contract.version !== "1.0.0") errors.push("content contract version 1.0.0 is required");
...
Object.values(contract.collections).filter(collection => collection.status === "implemented").forEach(collection => {
  ...
  items.forEach((item, index) => {
    ...
    if (Array.isArray(item.choices) && !item.choices.includes(item.answer)) {
      errors.push(`${label}: answer must be one of choices`);
    }
...
console.log(`CONTENT CONTRACT CHECK PASS: ${ids.size} items across UNIT 1-3`);
